"""오늘의 주제 기반 통합 학습: app/topics.py의 고정 주제 목록을 따라, 주제마다 핵심 단어를 미리
생성해 콘텐츠뱅크에 채운다 (1회성 스크립트, generate_grammar_curriculum.py와 동일한 목적).
단어학습/해석/회화가 전부 이 주제 단어 풀을 공유한다.

라우터(app/handlers/router.py)에도 "없으면 최초 1회 생성 후 캐싱" 폴백이 있어 이 스크립트 없이도
동작하지만, 실사용 전에 미리 돌려두면 사용자가 그날 처음 학습을 시작할 때 생성 대기 시간 없이
바로 단어카드를 받는다.

사용법:
    python scripts/generate_topic_words.py --count-per-topic 12
"""
...
from app.topics import DAILY_TOPICS  # noqa: E402
...
if __name__ == "__main__":
    ...
    parser.add_argument("--count-per-topic", type=int, default=12)
